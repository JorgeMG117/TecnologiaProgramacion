class Main
{
	public static void main(String[] args)
	{
		...
		Producto longaniza_graus = new Producto("Longaniza de Graus",0.5f,0.8f);
		contenedor_estandar.guardar(longaniza_graus);
		Producto adoquines_zaragoza = new Producto("Adoquines de Zaragoza",0.1f,5.0f);
		contenedor_estandar.guardar(adoquines_zaragoza);

		Toxico discos_melendi = new Toxico("Discos de Melendi",1f,10f);
		...
		SerVivo elvis_presley = new SerVivo("Elvis Presley",0.1f,100f);
		// Esto no deberia compilar
		...
		Producto apuntes_tepro = new Producto("Apuntes de Tecnologia de Programacion",1f,0.1f);
		if (!camion.guardar(apuntes_tepro))
			System.out.println("Camion lleno con apuntes de tepro");

		Producto trenzas_almudevar = new Producto("Trenzas de Almudevar",0.5f,0.8f);
